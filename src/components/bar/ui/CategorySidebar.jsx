...
export default function CategorySidebar({
  ...
  const handleCreate = () => {
    ...
    const newCategory = onAdd?.(newName.trim());
    setNewName("");
    setCreating(false);
    if (newCategory) onSelect?.(newCategory);
  };
  ...
  return (
    <div className="w-full h-full border-r border-white/10 flex flex-col bg-[#0e1628] p-4">

      {adminMode && (
        <button
          onClick={() => setCreating(true)}
          className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 transition rounded-xl px-3 py-2 mb-4 text-sm font-medium"
        >
      ...
      <div className="flex-1 overflow-y-auto space-y-2">
        ...
        {categories.map((cat) => (
          <div
            key={cat.id}
            className={`relative px-3 py-2 rounded-xl cursor-pointer transition ${
              selected?.id === cat.id
                ? "bg-indigo-600"
        ...
        {adminMode && creating && (
          <input
            ...
            className="w-full bg-white/5 rounded-xl px-3 py-2 outline-none border border-emerald-500 text-sm"
          />
        )}
